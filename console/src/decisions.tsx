...
export function DecisionInbox({ decisions = [], token = '', onChanged, compact = false, openDecisions, focusedDecisionId, navigation }: { decisions?: DecisionProjection[]; token?: string; onChanged?: () => Promise<void> | void; compact?: boolean; openDecisions?: () => void; focusedDecisionId?: string; navigation?: DecisionNavigation }) {
  ...
  return <section className={`decision-inbox ${compact ? 'decision-inbox-compact' : ''}`} aria-labelledby={compact ? 'overview-decisions-title' : 'decisions-title'}>
    <div className="panel panel-primary decision-inbox-heading"><div><span className="eyebrow">Owner decisions</span><h2 id={compact ? 'overview-decisions-title' : 'decisions-title'}>{title}</h2><p>{compact ? 'The most actionable observed decisions across projects.' : 'Resolve or delegate a specific observed problem. Manager acknowledgement remains a separate recorded state.'}</p></div><strong>{visible.length} shown</strong></div>
