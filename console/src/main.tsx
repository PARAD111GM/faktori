...
export function Overview({ state, runs, selectRun, openWork, filter }: { state: ConsoleState; runs: Run[]; selectRun: (id: string) => void; openWork?: () => void; filter?: ScopeFilter }) {
  ...
  const attentionLoops = loops.filter((loop) => loop.stale || loop.status === 'blocked' || loop.delivery?.issue || loop.delivery?.gates.some((gate) => gate.status === 'failed'));
  ...
  const gmState = state.factoryGM?.nightly;
  ...
  return <section className="workspace overview-workspace"><div className="overview-bento">
    <section className="panel panel-primary attention-panel"><div className="panel-heading"><div><h2>Needs your attention</h2><p>Observed requests, failures, and blocked work.</p></div><strong className="attention-count">{attentionRuns.length} runs · {attentionLoops.length} loops</strong></div>{attentionRuns.length > 0 && renderRuns(attentionRuns, '', '')}{attentionLoops.length > 0 && <div className="attention-loop-list">{attentionLoops.map((loop) => <a href="#work" key={loop.id}><strong>{loop.id}</strong><span>{loop.stale ? 'Recorded state stale' : loop.status === 'blocked' ? 'Blocked' : `${loop.delivery?.gates.find((gate) => gate.status === 'failed')?.label ?? 'Delivery gate'} failed`}</span>{loop.delivery && <small>Next: {loop.delivery.nextAction.label} · {stateLabel(loop.delivery.nextAction.role)}</small>}</a>)}</div>}{attentionRuns.length === 0 && attentionLoops.length === 0 && <Empty title="You’re all caught up" detail="No observed runs or Manager Loops in this scope need a decision or recovery." />}</section>
    <section className="panel capacity-panel"><div className="panel-heading"><div><h2>Coordinator capacity</h2><p>Coordinator resources with scoped run counts.</p></div><strong>{activeRuns} scoped runs</strong></div><dl className="capacity-list"><dt>Coordinator admission</dt><dd>{state.admissionPaused ? 'Paused' : 'Open'}</dd><dt>Scoped reservations</dt><dd>{count(reservedTokens)}</dd><dt>Coordinator reported usage</dt><dd>{resources?.reportedUsageCount ? measurement(resources.knownUsageTokens) : 'Unavailable'}</dd><dt>Coordinator unavailable measurements</dt><dd>{measurement(resources?.unavailableMeasurements ?? resources?.unavailableUsageCount)}</dd></dl><p className="capacity-note">Factory-wide registered-source usage is reported separately below. Unknown usage is never counted as zero.</p></section>
