...
export function Work({ state, runs, selectRun, openLoop, openRequest, openDecision, focusedRequestId, focusedSessionId, submit, submitManagerConnected }: { state: ConsoleState; runs: Run[]; selectRun: (id: string) => void; openLoop: (id: string) => void; openRequest: (id: string) => void; openDecision?: (id: string) => void; focusedRequestId?: string; focusedSessionId?: string; submit: (command: Command) => void; submitManagerConnected: (action: ManagerConnectedAction) => Promise<void> }) {
  ...
  return <section className="workspace"><div className="board-toolbar"><label>Project<select value={productId} onChange={event => setProductId(event.target.value)}><option value="">All projects</option>{projects.map(project => <option key={project.id} value={project.id}>{project.name}</option>)}</select></label></div><TicketBoard workManagement={state.workManagement} boards={state.jiraBoards ?? []} runs={runs} filter={filter} selectRun={selectRun} /><ManagerConnected snapshot={state.managerConnected} workManagement={state.workManagement} focusedRequestId={focusedRequestId} focusedSessionId={focusedSessionId} submit={submitManagerConnected} /><div className="panel panel-primary section-header work-header"><div><span className="eyebrow">Coordinator execution</span><h2>Execution runs</h2><p>Agent executions are separate from tickets. A ticket appears above even before an agent starts it.</p></div><form className="start-work" onSubmit={(event) => { event.preventDefault(); if (workItemId.trim()) { submit({ type: 'start_work', workItemId: workItemId.trim() }); setWorkItemId(''); } }}><label>Eligible work ID<input value={workItemId} onChange={(event) => setWorkItemId(event.target.value)} placeholder="Enter an approved work item ID" /></label><button className="primary" type="submit" disabled={state.admissionPaused}>Start work</button></form></div>
  <div className="board-toolbar"><label>Find work<input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search work ID, provider, or status" /></label><span role="status">{matchingRuns.length} of {scopedRuns.length} runs{query && <button type="button" onClick={() => setQuery('')}>Clear search</button>}</span></div>
...
function App() {
  ...
  return <main className="app-shell">
    ...
    <aside className="sidebar"><a className="wordmark" href="#overview" onClick={() => setView('overview')}><img className="brand-logo" src={faktoriLogo} alt="" /><span className="brand-name">FAKTORI</span></a><nav aria-label="Console views">{primaryViews.map((item) => <button key={item} type="button" className={view === item ? 'active' : ''} aria-current={view === item ? 'page' : undefined} onClick={() => { if (item === 'projects') setSelectedProjectId(undefined); setView(item); }}><Icon name={item} /><span>{item}</span></button>)}</nav></aside>
