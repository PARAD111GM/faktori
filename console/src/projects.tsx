...
function Ticket({ ticket, context, requests, navigation }: { ticket: WorkManagementTicket; context: TicketContext; requests: WorkManagementRequest[]; navigation: DetailNavigation }) {
  ...
  return <li className="work-ticket">
    <div className="ticket-heading"><span className="ticket-number">{ticket.order}</span><div><HelpHeading level={4} scope="project-ticket">{ticket.title}</HelpHeading><p>{ticket.goal}</p></div>{ticket.issueKey && <small className="ticket-issue">{ticket.issueKey}</small>}</div>
...
function Phase({ phase, context, requests, navigation }: { phase: WorkManagementPhase; context: Omit<TicketContext, 'phaseId'>; requests: WorkManagementRequest[]; navigation: DetailNavigation }) {
  ...
  return <li className="work-phase">
    <div className="phase-heading"><span className="phase-number">{phase.order}</span><div><HelpHeading level={4} scope="project-phase">{phase.title}</HelpHeading><p>{phase.goal}</p></div></div>
...
function Progress({ progress }: { progress?: WorkManagementProgress }) {
  if (!progress) return <section className="project-progress project-progress-unavailable"><HelpHeading level={3} scope="projects">Qualified progress</HelpHeading><p>Unavailable — no explicit catalog ticket-status population was published.</p></section>;
...
export function Project({ project, requests, sessions, navigation, onBackToProjects, onRefresh }: { project: WorkManagementProjectProjection; requests: WorkManagementRequest[]; sessions: WorkManagementSession[]; navigation: DetailNavigation; onBackToProjects?: () => void; onRefresh?: () => void }) {
  ...
  return <article className="project-card">
    ...
    <header className="project-heading"><div><span className="eyebrow">Project</span><HelpHeading level={2} scope="projects">{project.title}</HelpHeading><p>{project.goal}</p></div><small>{project.productId}</small></header>
    <Progress progress={project.progress} />
    <DailySummary summary={project.dailySummaries?.at(-1)} />
    <PullRequests project={project} pullRequests={project.pullRequests} />
    {project.artifacts.length > 0 && <section className="project-artifacts" aria-label={`${project.title} artifacts`}><HelpHeading level={3} scope="projects">Artifacts</HelpHeading>{project.artifacts.map((artifact) => <Artifact key={artifact.id} artifact={artifact} />)}</section>}
    {project.plans.length === 0 ? <div className="empty"><HelpHeading level={3} scope="projects">No plans published</HelpHeading><p>This project has no ordered plan in the current catalog.</p></div> : <div className="project-plans">{ordered(project.plans).map((plan) => <section className="work-plan" key={plan.id}><header><span className="plan-number">Plan {plan.order}</span><div><HelpHeading level={3} scope="project-plan">{plan.title}</HelpHeading><p>{plan.goal}</p></div></header>{plan.phases.length === 0 ? <p className="quiet">No phases recorded for this plan.</p> : <ol className="work-phases">{ordered(plan.phases).map((phase) => <Phase key={phase.id} phase={phase} context={{ productId: project.productId, planId: plan.id, dependencyTitles, sessions }} requests={requests} navigation={navigation} />)}</ol>}<small className="work-id">Plan ID: {plan.id}</small></section>)}</div>}
  </article>;
}
...
export function Projects({ workManagement, navigation, selectedProjectId, onSelectProject, onBackToProjects, onRefresh }: { workManagement?: WorkManagementState; navigation: DetailNavigation } & ProjectNavigation) {
  if (!workManagement || workManagement.status === 'unavailable') {
    const missing = !workManagement?.error || workManagement.error === 'work_catalog_not_configured';
    return <section className="workspace projects-workspace"><div className="panel panel-primary"><div className="panel-heading"><div><HelpHeading level={2} scope="projects">{missing ? 'Connect your project plans' : 'Project plans could not be loaded'}</HelpHeading><p>The Console needs a project index to show your plans, phases, tickets, and saved artifacts.</p></div><span className="state state-unavailable">{missing ? 'Not connected' : 'Needs attention'}</span></div><div className="empty"><p>{missing ? 'Your work has not been deleted. This Console has not been told where its project index lives.' : 'The configured project index is not readable or valid. Your source files are not changed by this error.'}</p><p>Ask your coding agent to {missing ? 'connect the existing project index using the workCatalog setting in this Console’s configuration' : 'check the workCatalog file path and repair the reported validation error'}. A project index is a local file, not a Jira login or an AI provider account. Restart only this Console if the configuration changes, when it is safe to do so.</p><p className="quiet">The factory floor is still there; this is the missing noticeboard.</p>{workManagement?.error && <details><HelpSummary scope="projects">Technical details for your agent</HelpSummary><code>{workManagement.error}</code></details>}</div></div></section>;
  }
  ...
  return <section className="workspace projects-workspace">
    ...
    {workManagement.projects.length === 0 ? <div className="panel panel-support"><div className="empty"><HelpHeading level={3} scope="projects">No projects published</HelpHeading><p>The configured catalog did not publish a project projection.</p></div></div>
