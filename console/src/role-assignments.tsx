...
export function RoleAssignments({ assignments = [], onChange, settings }: { assignments?: RoleAssignment[]; onChange?: (next: RoleAssignment[]) => void; settings: ConsoleSettings }) {
  ...
  return <section className="role-assignments" aria-label="Role assignments">
    <HelpHeading level={3} scope="role-assignments">Role assignments</HelpHeading><p>Choose the provider, model, and standing instructions for each responsibility. Unlabelled work uses builder. Products and pods inherit these assignments unless overridden; roles and prompts do not grant permissions.</p>
    ...
    </> : <><strong>{assignment.role.replaceAll('-', ' ')}</strong><span>{assignment.providerId}</span><span>{assignment.model || 'Work plan model'}</span><small>{assignment.reasoning ? `${assignment.reasoning} reasoning` : 'Work plan / provider-default reasoning'}</small></>}
      {editing ? <label className="role-prompt">Role Prompt<textarea aria-label={`Role ${index+1}: Role Prompt`} value={assignment.rolePrompt ?? ''} rows={4} maxLength={16000} placeholder="Describe this role’s responsibilities, working rules, and expected handoff." onChange={(event) => update(index, {rolePrompt: event.target.value || undefined})} /><small>Included in dispatches that use this assignment. Keep the standing orders concise; approvals still outrank the foreman. Do not put credentials here. Saved changes take effect after the Console restarts.</small></label> : <div className="role-prompt"><strong>Role Prompt</strong><p>{assignment.rolePrompt || 'No additional role instructions.'}</p></div>}
    </div>)}</div>
