...
Adds `faktori.preflight-result/v1`, `faktori preflight <request.json>`, and the same sanitized projection in the Factory Console. Checks cover configuration, Console, execution, resources, provider capabilities, integrations, and live evidence with stable IDs, scope, basis, freshness, and fixed remediation. Public JSON success assertions remain explicitly unverified and cannot produce ready, execution-ready, or live-verified claims; failures dominate contradictory provider observations. The evaluator is data-only and imports no filesystem, process, provider, coordinator, provisioning, or network execution surface.
