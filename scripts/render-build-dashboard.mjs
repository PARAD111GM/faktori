...
function page(checklist, summary) {
  ...
  const blockerItems = blockers.length === 0 ? '<li>None</li>' : blockers.map((ticket) => `<li>${escapeHtml(ticket.id)} — ${escapeHtml(ticket.title)}<p>${escapeHtml((ticket.notes ?? []).join(" "))}</p></li>`).join('');
  ...
  const actualLabel = usage.actual?.kind === 'overlap-safe-lower-bound'
    ...
    : 'Measured actual';
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Faktori construction dashboard</title><style>body{font:16px system-ui;margin:2rem;max-width:1100px;color:#17212b}h1{margin-bottom:.2rem}section{margin:2rem 0;padding:1.25rem;border:1px solid #d5dce2;border-radius:.6rem}table{border-collapse:collapse;width:100%}th,td{padding:.6rem;border-bottom:1px solid #d5dce2;text-align:left;vertical-align:top}.bar{height:1rem;background:#e8edf1;border-radius:999px}.bar i{display:block;height:100%;background:#156c4b;border-radius:inherit}.usage i{background:#315c9b}.status{font-weight:600}.blocked{color:#a43b00}.complete{color:#156c4b}svg{width:100%;height:auto;background:#f7f9fa}.events{max-height:10rem;overflow:auto}small{color:#526170}</style></head><body><main><h1>Faktori construction</h1><p>Current phase: ${escapeHtml(phase)}. ${complete} of ${tickets.length} complete (${progress}%).</p><p>${escapeHtml(checklist.project?.progressSummary ?? "")}</p><div class="bar" aria-label="Checklist progress"><i style="width:${progress}%"></i></div><section><h2>Completion history</h2>${chart(historySeries(history), tickets.length)}</section><section><h2>Phase usage</h2><p>${actualLabel}: ${escapeHtml(usage.actual?.total ?? 'unknown')} tokens. Estimated work: ${escapeHtml(usage.estimated?.total ?? 'unknown')} tokens. Phase budget: ${escapeHtml(budget ?? 'unknown')} tokens. ${metric('Remaining allowance', usage.budget?.remaining)}. Budget used: ${escapeHtml(usagePercent ?? 'unknown')}%.</p><p>${metric('Input', usage.actual?.input)}. ${metric('Output', usage.actual?.output)}. ${metric('Cached', usage.actual?.cached)}. ${metric('Reasoning', usage.actual?.reasoning)}.</p><div class="bar usage" aria-label="Token budget consumption"><i style="width:${chartWidth}%"></i></div><p>Notices: ${escapeHtml((usage.notices ?? []).map((notice) => `${notice.threshold}%`).join(', ') || 'none')}. Model/reasoning records: ${escapeHtml(modelRecords)}.</p><h3>Missing usage telemetry</h3><ul>${missingItems}</ul></section><section><h2>Blockers</h2><ul>${blockerItems}</ul></section><section><h2>Full checklist</h2><table><thead><tr><th>ID</th><th>Ticket</th><th>Status</th><th>Dependencies</th><th>Owner</th><th>Model/reasoning</th></tr></thead><tbody>${rows}</tbody></table></section></main></body></html>`;
}
