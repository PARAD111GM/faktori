...
it('shows every local ticket without needing an execution run or Jira', () => {
  const html = render(); expect(html).toContain('Save a task'); expect(html).toContain('Show saved tasks'); expect(html).toContain('Status not recorded'); expect(html).toContain('Local tickets');
});
